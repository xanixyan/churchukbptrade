...
export default function CheckoutModal({
  ...
  const isFormValid = useMemo(() => {
    return (
      ...
      offer.trim().length > 0 &&
      offer.trim().length <= 500 &&
      selections.length > 0
    );
  }, [discordNick, offer, selections]);
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  rows={2}
                  disabled={formState === "submitting"}
                  className="w-full px-3 py-2 bg-dark-700 border border-dark-600 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-neon-cyan transition-colors resize-none disabled:opacity-50"
                />
              </div>
